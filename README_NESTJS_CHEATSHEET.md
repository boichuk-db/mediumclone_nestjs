...
```ts
@Module({
  imports: [UserModule, ArticleModule, ProfileModule, TypeOrmModule.forRoot(...)]
})
...
### Кастомна помилка для єдиного формату API

У цьому проєкті використовується `BackendException`:
